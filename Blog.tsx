...
export const Blog: React.FC<BlogProps> = ({ onNavigate }) => {
  ...
  const handleLinkClick = (e: React.MouseEvent, href: string) => {
    ...
    if (href.startsWith('#')) {
      ...
      if (Object.values(ViewState).includes(viewKey as ViewState)) {
          ...
          setSelectedPost(null); // Return to list view or keep post open? Usually navigating away implies closing post view in this single-page app context.
      }
    }
  };
  ...
  const renderContent = (content: string) => {
    return content.split('\n').map((paragraph, pIdx) => {
        ...
        while ((match = regex.exec(paragraph)) !== null) {
            ...
            parts.push(
                <a 
                    ...
                    className="inline-flex items-center gap-1 mx-1 text-primary font-bold hover:underline cursor-pointer bg-primary/5 px-2 py-0.5 rounded-md hover:bg-primary/10 transition-colors"
                >
        ...
        return <p key={pIdx} className="mb-4 text-gray-700 leading-relaxed">{parts}</p>;
    });
  };
